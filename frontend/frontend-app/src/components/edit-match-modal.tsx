...
interface EditMatchModalProps {
  match: Match
  onClose: () => void
  onSave: (matchId: string, dateTime: string) => void
}

export default function EditMatchModal({ match, onClose, onSave }: EditMatchModalProps) {
  const [dateTime, setDateTime] = useState<string>(match.date || "")

  const handleSubmit = (e: React.FormEvent) => {
    e.preventDefault()
    if (dateTime) {
      onSave(match.id, dateTime)
    }
  }
          ...
          <div className="bg-white rounded-lg p-6">
            <div className="text-center mb-6">
              <div className="flex items-center justify-center gap-4">
                <span className="text-2xl font-bold text-primary">{match.homeTeam}</span>
                <span className="text-2xl font-bold text-gray-400">:</span>
                <span className="text-2xl font-bold text-primary">{match.awayTeam}</span>
              </div>
            </div>
